...
<c:forEach items="${orderInfos}" var="orderInfo">
    <div id="div<%=i%>">
        订单号：<span>${orderInfo.getOrderId()}</span><br>
        图书号：<span>${orderInfo.getBookId()}</span><br>
        状态：<span>${orderInfo.getOrderState()}</span><br>
        <c:if   test="${orderInfo.getOrderState()==2}">
            <button type="button" onclick="getGoods(${orderInfo.getBookId()},${orderInfo.getOrderState()},<%=i%>)">我已发货</button><br>
        </c:if>
    </div>
    <%i++;%>
</c:forEach>
