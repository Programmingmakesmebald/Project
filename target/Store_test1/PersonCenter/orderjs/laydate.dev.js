...
var Dates = {}, doc = document, creat = 'createElement', byid = 'getElementById', tags = 'getElementsByTagName';
    var as = ['laydate_box', 'laydate_void', 'laydate_click', 'LayDateSkin', '../css/laydate/skins/', '/laydate.css'];
...
//主接口
    win.laydate = function (options) {
        ...
        if(options.callbackFun && typeof(options.callbackFun)=="function"){
            config.callbackFun = options.callbackFun;
        }
        try {
...
//补齐数位
    ...
    Dates.stopmp = function (e) {
        e = e || win.event;
        e.stopPropagation ? e.stopPropagation() : e.cancelBubble = true;  //阻止当前事件在DOM树上冒泡
        return this;
    };
...
//关闭控件
    Dates.close = function () {
        ...
        Dates.elem = null;
        if(config.callbackFun && typeof(config.callbackFun)=="function"){
            config.callbackFun();
        }
    };
    ...
    Dates.events = function () {
        ...
        Dates.each(S('#laydate_table td'), function (i, elem) {
            Dates.on(elem, 'click', function (ev) {
                ...
                if(this.getAttribute('d')>current_day || this.getAttribute('m')>current_month)return;
                if (!Dates.hasClass(this, as[1])) {
                    Dates.stopmp(ev);
